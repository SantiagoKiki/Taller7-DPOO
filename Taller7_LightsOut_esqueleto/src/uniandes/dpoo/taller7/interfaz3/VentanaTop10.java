...
public class VentanaTop10 extends JDialog  {
		...
		public VentanaTop10(Collection<RegistroTop10> regis) {
		...
		public DefaultListModel<JPanel> crearModelo(Collection<RegistroTop10> regis) {
			...
			for(int i = -1; i < registros.length; i++) {
				...
				if (i == -1) {
					...
					numero = new JLabel(String.valueOf(i+1));
					jugador = new JLabel(registro.darNombre());
					puntaje = new JLabel(String.valueOf(registro.darPuntos()));
				}
				panel.add(numero);
				panel.add(jugador);
				panel.add(puntaje);
